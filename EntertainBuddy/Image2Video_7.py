...
def ErrorCorrection():
    ...
    gif = VideoFileClip("/home/sasi/Downloads/diwali-sparkles-stars.gif")
...
def ImageAnimation(ImageClip, duration, i, flag):
    # Load the image
    image = ImageClip
    ...
    if i == 0:
        stock_path = os.path.dirname(__file__) + "/Background/Stock" + str(random.randint(1, 5)) + ".mp4"
        print(stock_path)
        stock = VideoFileClip(stock_path)
        stock = CompositeVideoClip([background.set_duration(stock.duration), stock])
        final = concatenate([final, stock], method="compose")
    return final
...
def MakeVideo():
    ...
    for i in range(shift_count):
        ...
        img = ImageAnimation(img, divider, i, flag="L" if i % 2 == 0 else "R")
        # img = img.crossfadein(1.0).crossfadeout(1.0)
        final.append(img)

    # Color clip with black and reels size
    background = ColorClip(color=(0, 0, 0), size=(1080, 1920))
    background = ImageClip(os.path.dirname(__file__) + "/Background/Background_Template_Dark.png").set_duration(
        shift_count * divider)
    ...
    text1 = TextClip(parts_of_meme, font="Montserrat-ExtraBold", color="white", method="caption",
                     size=(1080 - 18 - 18, 200),
                     align="south", kerning=-5).set_position((18, 420)).set_duration(divider * shift_count)
...
date = "20240403"#.join(str(datetime.date.today()).split("-"))
